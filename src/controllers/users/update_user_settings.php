...
if ($_POST['referer'] == 'profile.php') {
    ...
    $update_user_query = "UPDATE users SET color_scheme = ?, note_order = ?, hide_alerts = ? WHERE id = ?";
    $update_user_stmt = mysqli_prepare($database, $update_user_query);
    mysqli_stmt_bind_param($update_user_stmt, "ssii", $color_scheme, $note_order, $hide_alerts, $user_id);
