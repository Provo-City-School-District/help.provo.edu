...
// Still crusty, but seems to work better than prior.
...
if (isset($_SESSION['last_note']) && $_SESSION['last_note']['ticket_id'] === $ticket_id && $_SESSION['last_note']['content'] === $note_content) {
    echo json_encode(['status' => 'duplicate', 'message' => 'Duplicate note detected']);
    exit;
}
...
if ($add_note_result) {
    ...
    //added these messages to json to use them later, but currently we aren't using them
