...
$client->setClientSecret(getenv('GMAIL_SECRET'));
$client->setRedirectUri(getenv('GMAIL_REDIRECT'));
...
$imap2_path = '{imap.gmail.com:993/imap/ssl}INBOX';
$username = getenv("GMAIL_USER");
$password = getenv("GMAIL_PASSWORD");

$mbox = imap2_open($imap2_path, $username, $access_token, OP_XOAUTH2) or die('Cannot connect to Gmail: ' . imap2_last_error());
$msg_count = imap2_num_msg($mbox);
if ($msg_count == false) {
    log_app(LOG_ERR, "IMAP message count failed to query: " . imap2_last_error());
}

// Sort by msg date
imap2_sort($mbox, SORTDATE, false);
...
{
    $header = imap2_fetchheader($mailbox, $msg_id);
...
for ($i = 1; $i <= $msg_count; $i++) {
    $header = imap2_headerinfo($mbox, $i);
    if (!$header) {
    ...
    if (in_array($sender_email, $blacklisted_emails)) {
        ...
        $succeeded_uids[] = imap2_uid($mbox, $i);
        continue;
    }
    ...
    $obj_structure = imap2_fetchstructure($mbox, $i);
    ...
    $text = imap2_fetchbody($mbox, $i, $section);

    if ($obj_section->encoding == 3) {
        $text = imap2_base64($text);
    } else if ($obj_section->encoding == 4) {
        $text = imap2_qprint($text);
    }
...
foreach ($succeeded_uids as $uid) {
    ...
    if ($move_emails_after_parsed) {
        $msg_move_result = imap2_mail_move($mbox, $uid, "[Gmail]/Important", CP_UID);
        if (!$msg_move_result) {
            log_app(LOG_ERR, "Failed to move message: " . imap2_last_error());
        } else {
            $moved_emails++;
        }
    }
...
function find_and_upload_attachments(int $ticket_id, IMAP\Connection $mbox, int $msg_num, $sender_username)
{
    ...
    /* get mail structure */
    $structure = imap2_fetchstructure($mbox, $msg_num);
    ...
    if (isset($structure->parts) && count($structure->parts)) {
        for ($i = 0; $i < count($structure->parts); $i++) {
            ...
            if ($attachments[$i]['is_attachment']) {
                $attachments[$i]['attachment'] = imap2_fetchbody($mbox, $msg_num, $i + 1);
